  ...
  const { data } = await axios.post<Color[]>(
    ...
    `step=${step}&sort=new&tags=&timeframe=4000`,
    {
      headers: {
        ...
        'cookie': '_ga=GA1.2.649898212.1649879639; _gid=GA1.2.1192212843.1650533076; _gat=1',
...
const fetchColor = async (id: string): Promise<Color | null> => {
  try {
    const { data } = await axios.post<[Color]>('https://colorhunt.co/php/single.php', `single=${id}`, {
      headers: {
        'accept': 'text/html, */*; q=0.01',
        'accept-language': 'en,ar;q=0.9',
        'content-type': 'application/x-www-form-urlencoded; charset=UTF-8',
